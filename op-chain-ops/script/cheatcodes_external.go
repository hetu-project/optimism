...
func lookupKeys(v any, query []string) ([]string, error) {
	...
	switch x := v.(type) {
	case []any:
		index, err := strconv.ParseUint(q, 10, 64)
		if err != nil {
			return nil, fmt.Errorf("invalid query part: %q", q)
		}
		if index >= uint64(len(x)) {
			return nil, fmt.Errorf("index %d larger than length %d", index, len(x))
		}
		return lookupKeys(x[index], query[1:])
	case map[string]any:
		if q == "$" {
			if len(query) > 1 {
		...
		data, ok := x[q]
		if !ok {
			return nil, fmt.Errorf("unknown key %q", q)
		}
		return lookupKeys(data, query[1:])
	default:
...
func (c *CheatCodesPrecompile) ParseJsonKeys(data string, key string) ([]string, error) {
	...
	return lookupKeys(x, strings.Split(key, "."))
}
